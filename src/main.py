...
def login_screen():
  ...
  login_btn = styles.BlueButton(foreground.foreground, "Log In", command=lambda: login.log_in(styles.selected_mode), sizex=500, sizey=200)
  login_btn.show(1000,750)
 
  #if the user clicks the Log in button:
    #trigger the log in function
  signup_btn = styles.GreenButton(foreground.foreground, "New Account", command=lambda: signup.signup(styles.selected_mode), sizex=500, sizey=200)
